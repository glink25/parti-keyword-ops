...
app.addEventListener('click', (event) => {
  const target = (event.target as HTMLElement).closest<HTMLElement>('[data-action], [data-card]');
...
app.addEventListener('submit', (event) => {
  if ((event.target as HTMLElement).id !== 'clueForm') return;
  event.preventDefault();
  const word = (document.querySelector<HTMLInputElement>('#clueWord')?.value ?? '').trim();
  const count = Number(document.querySelector<HTMLSelectElement>('#clueCount')?.value ?? 1);
...
parti.onEvent('keywordOps:error', (payload) => showToast(String((payload as { message?: unknown })?.message ?? '操作无效'));
